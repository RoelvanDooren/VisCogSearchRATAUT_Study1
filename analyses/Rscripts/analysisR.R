...
packages <- c("stringr", "dplyr", "tidyr", "ggplot2", "plyr", "Rmisc", "boot", "plotrix")
ipak(packages)
...
# Copy datafiles ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
scrabblefiles <- list.files('../../output', pattern = '_scrabble_', full.names = T, recursive = T)
foragingfiles <- list.files('../../output', pattern = '_visual_foraging', full.names = T, recursive = T)
autfiles <- list.files('../../output', pattern = '_alternate_uses_', full.names = T, recursive = T)
ratfiles <- list.files('../../output', pattern = '_remote_association_', full.names = T, recursive = T)
...
# Recreate foraging paths ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#source("recreatePathsR.R") #Uncomment for movie creation

# Recreate foraging environment ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
all.datafiles <- list.files('../Rawdata/Foraging/', pattern = '_visual_foraging_mapSurface_trial_', full.names = T, recursive = T)
file.copy(all.datafiles, './')
shell("recreateMap.py")

files <- list.files('./', pattern = '_visual_foraging_mapSurfaceNEW_trial_', full.names = T, recursive = T)
pdf('../Output/foraging_environments_by_participant.pdf')
for (currentFile in files) {
  dat <- read.csv2(currentFile, sep = ',', stringsAsFactors = FALSE, header = FALSE)
  colnames(dat) <- paste("col", 1:200, sep = "")
  dat <- as.matrix(dat)
  image(dat, col = c("black", "green"), width = 50, height = 50)
  title(paste0("Participant: ", str_match(currentFile, '([0-9]+)([0-9]+)')[[1]], " Trial: ", str_sub(currentFile, -5, -5), " Condition: ", ifelse(as.numeric(str_match(currentFile, '([0-9]+)([0-9]+)')[[1]]) %% 2 == 1, "Clumpy", "Diffuse")))
}
dev.off()
file.remove(list.files(pattern = '_visual_foraging_', full.names = T))

# Clear workspace
rm(dat, all.datafiles, currentFile, files, foragingfiles, autfiles, ratfiles, scrabblefiles)

# Load and clean datafiles ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# Foraging data
foraging_df <- data.frame()
files <- list.files("../Rawdata/Foraging/", pattern="visual_foraging.txt", recursive=TRUE, full.names=TRUE)
for (currentFile in files) {
...
files <- list.files("../Rawdata/Scrabble", pattern="scrabble_pretest.txt", recursive=TRUE, full.names=TRUE)
for (currentFile in files) {
...
files <- list.files("../Rawdata/Scrabble", pattern="scrabble_posttest.txt", recursive=TRUE, full.names=TRUE)
for (currentFile in files) {

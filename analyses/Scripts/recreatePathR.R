...
packages <- c("ggplot2", "stringr", "dplyr", "tidyr", "rPython")
ipak(packages)

# Copy datafiles ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
scrabblefiles <- list.files('../../output', pattern = '_scrabble_', full.names = T, recursive = F)
foragingfiles <- list.files('../../output', pattern = '_visual_foraging', full.names = T, recursive = F)
autfiles <- list.files('../../output', pattern = '_alternate_uses_', full.names = T, recursive = F)
ratfiles <- list.files('../../output', pattern = '_remote_association_', full.names = T, recursive = F)
...
# Transpose arrays from wide to long format
#shell("transposePath.py") # Note: only works on Windows
system('python transposePath.py')
...
# Clean up workspace.
rm(area_visited, eachnum, eachrow, eachvalue, filterStart, pb1, f, frame_to_export, dat, all.datafiles,
...
# Call python script to recreate map surfaces
#shell("recreateMap.py") # Note: Only works for Windows
system('python recreateMap.py')
...
# Clean up workspace
rm(dat, all.datafiles, currentFile, files, nonprocesseddata, numplots, pb1, processed.data, processed.files, f)
...
for (f in select.files) {
  ...
  if (str_detect(f, 'practice')) {
    ...
    # Create frames for movie
    for (i in seq(1, length(dat$resource_encountered), 200)) {
      png(filename = sprintf(paste0('./Temporary/ERC_WP3_Year1_Study1_', str_match(f, '([0-9]+)([0-9]+)')[[1]], '_', 
                                       str_match(f, '([0-9]+)([0-9]+)([0-9]+)([0-9]+)([0-9]+)')[[1]], 
